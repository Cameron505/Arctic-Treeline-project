---
title: "FTICR_Field_Treeline_Polar_CKM"
...
```{r}
dat_edata <- read.csv(file="Field_Polar_Edata.csv", header=TRUE, sep=",")
```

## f_data (Sample Data)
```{r}
dat_fdata <- read.csv(file="Field_Polar_Fdata.csv", header=TRUE, sep=",")
...
```{r }
dat_emeta <- read.csv(file="Field_Polar_Emeta.csv", header=TRUE, sep=",")
...
PCA3<- merge(Sample.KeyValues2, data,by.x="ID", by.y="ID", sort = TRUE)

PCA3[, c(4:17)] <- sapply(PCA3[, c(4:17)], as.numeric)

```
##Plotting Stat Ellipises on PCA plot
```{r}
PCA3$Year<-as.factor(PCA3$Year)
ggplot(PCA3, aes(x=Axis.1, y=Axis.2, color=Year))+
  geom_point()+
  stat_ellipse(aes(x=Axis.1,y=Axis.2, level=0.95))+
  theme_bw()
```
